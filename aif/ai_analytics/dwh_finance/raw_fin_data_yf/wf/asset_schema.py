"""Schema creation assets for the RAW_FIN_DATA_YF schema.

This module defines Dagster assets responsible for creating the RAW_FIN_DATA_YF schema
...
The schema serves as the initial data ingestion layer for financial data from Yahoo Finance,
providing raw storage for stock prices and other financial information before any
transformation or normalization occurs.
"""
...
@dg.asset(
    ...
    name="SCHEMA",
    description="Raw financial data from Yahoo Finance",
    group_name=f"{DWH_NAME}_{SCHEMA_NAME}",
)
def asset_schema(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    """Create the RAW_FIN_DATA_YF schema in the DWH Finance database.

    This asset is responsible for creating the schema that will store raw financial data
